...
This work has been developed as part of a Master's thesis to implement data structure tuning. Additionally, work on data structure replacement was started but abandoned in favor of data structure tuning. All code-related material has been included in this repository, under the following folders:

* [`datastructrepl/`](substratevm/src/com.oracle.svm.hosted/src/com/oracle/svm/hosted/datastructrepl/) contains all optimization code relating to both tuning and replacement.
* [`idealgraphvisualizer/`](idealgraphvisualizer/) contains a complete copy of IGV to view GraalIR graphs as they are generated by Native Image.
* [`TestCode/`](TestCode/) contains all code used for debugging and testing of this work, including the `tstime` tool. The [`run`](TestCode/run.sh) script contains all of the instructions and calls necessary to run every aspect of testing. Be aware that you will need to export the name of the package/main class before running any of the optimization scripts. [`TuningSynthetic/`](TestCode/PerformanceTesting/TuningSynthetic) contains the tuning microbenchmark. `tstime` is simply compiled with `make` inside the main [`TestCode/`](TestCode/) folder.
* _You will need to set GraalVM as your Java. Take note all filepaths are absolute due to a quirk of my installation. You will need to refactor all of them. Searching for_ roman _or_ thesis _should allow you to find them easily_.
* This work is provided as-is, and I do not expect to update it or keep it at parity with the main graal branch. Any conflict resolution will be up to the user of this repository. If you have any questions, feel free to contact me at `roman0308@hotmail.com`.
